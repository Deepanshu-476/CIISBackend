...
function controller(model = {}, used = null) {
  ...
  vm.runInNewContext(fs.readFileSync(require.resolve('../controllers/leadTypeController'), 'utf8'), {
    module, exports: module.exports,
    require: name => name === '../models/LeadType' ? model : {
...
test('normalizes names, accepts requests without sort order and validates status', () => {
  ...
  assert.equal(validate({name:'  Test   Series ',sort:0}).normalizedName, 'test series');
  assert.equal(validate({name:'NEET',status:'Inactive'}).status, 'Inactive');
  assert.equal('sort' in validate({name:'NEET'}), false);
  assert.throws(() => validate({name:' ',sort:1}));
  assert.throws(() => validate({name:'NEET',status:'unknown'}));
});
test('create uses authenticated company and creator, ignoring submitted ownership', async () => {
  ...
  const res = response();
  await c.create({body:{name:'NEET',company:'other',createdBy:'other',isSystem:true},leadTypeCompany:'own',user:{id:'me'}},res);
...
test('update is scoped to authenticated company and returns 404 for unavailable records', async () => {
  ...
  const res=response();
  await c.update({params:{id:'507f1f77bcf86cd799439011'},body:{name:'NEET'},leadTypeCompany:'own'},res);
...
test('duplicate database constraint returns a useful conflict', async () => {
  ...
  await c.create({body:{name:'NEET'},leadTypeCompany:'own',user:{id:'me'}},res);
  assert.equal(res.code,409);
});
test('used types cannot be deleted and system types are excluded atomically', async () => {
  const req={params:{id:'507f1f77bcf86cd799439011'},leadTypeCompany:'own'};
...
test('lead type permissions enforce company enablement and page action grants', async () => {
  const user='507f1f77bcf86cd799439011'; const company='507f1f77bcf86cd799439012';
  let allowedPages=['admin-crm-lead-types'];
  const module={exports:{}};
  vm.runInNewContext(fs.readFileSync(require.resolve('../middleware/leadTypePermission'),'utf8'), {
    module,
    require: name => name === 'mongoose' ? mongoose : name === '../models/Company'
      ...
      : name === './crmPagePermission' ? { requireCrmPagePermission: (path, action) => (req, res, next) => {
        assert.equal(path, '/ciisUser/crm/admin/lead-types'); assert.ok(['view', 'edit', 'delete'].includes(action)); next();
  ...
  for (const action of ['view','edit','delete']) {
    ...
    assert.equal(req.leadTypeCompany,company);
  }
