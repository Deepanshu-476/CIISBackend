...
exports.createLeavePolicy = async (req, res, next) => {
  try {
    ...
    await leavePolicy.populate([{ path: "department", select: "name branch" }, { path: "jobRoles", select: "name department" }]);
...
exports.updateLeavePolicy = async (req, res, next) => {
  try {
    ...
    ).populate("department", "name branch").populate("jobRoles", "name department");
