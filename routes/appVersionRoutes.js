...
const getFallbackSettings = () => {
  ...
  const iosLatestVersionName = process.env.IOS_LATEST_VERSION_NAME || '1.1.22';
  const androidPackageName = process.env.ANDROID_PACKAGE_NAME || 'ciisnetwork.in';
  const androidLatestVersionName = process.env.ANDROID_LATEST_VERSION_NAME || '1.1.22';

  return {
    ios: {
      latestVersionName: iosLatestVersionName,
      latestVersionCode: parseNumber(process.env.IOS_LATEST_BUILD_NUMBER, 37),
    ...
    android: {
      ...
      latestVersionCode: parseNumber(process.env.ANDROID_LATEST_VERSION_CODE, 33),
